...
const (
	...
	CONFIG_URL_POSTGRESQL = "/configs/default/application/python.PostgreSQL"
)
...
func Init() {
	...
	viper.AddConfigPath("accountserver/jsonconfig")
...
func initDatabaseConfig(host string, port string, user string, password string, dbname string) {
	...
	mysql.ConfigDB(host, port, user, password, dbname)
}
