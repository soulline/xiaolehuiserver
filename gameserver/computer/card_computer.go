...
*order==3 底牌次序
 */
func Dispatcher(order int, vals []string) []string {
		...
		if len(maxValues) == alive {
			if len(maxValues) == 1 {
				...
				if IsContinuity(mapCard, false) {
					cardShow.CompareCount = len(maxValues)
...
* 删除牌
 */
func RemoveCard(cardStr string, cards []string) (bool, []string) {
	if len(cards) <= 0 {
		return false, nil
	} else {
		var removeIndex int
		...
		cards = append(cards[:removeIndex], cards[removeIndex+1:]...)
		fmt.Println(cards)
		return true, cards
	}
}
